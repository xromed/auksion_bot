...
MIN_LOT_ID        = 10_000_000  # лоты с меньшим ID — устаревшие, некорректно отображаются на сайте
# ============================================================
...
def to_usd(amount, rate: float) -> str:
    try:
        val = round(float(amount) / rate)
        return f"$ {val:,}".replace(",", " ")  # неразрывный пробел как разделитель тысяч
    except:
        return "—"
